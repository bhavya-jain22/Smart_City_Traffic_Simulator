...
public class SimulationPanel extends JPanel {
    ...
    private final Color BG_DARK = new Color(24, 24, 36);
    private final Color ROAD_NORMAL = new Color(60, 60, 75);
    private final Color ROAD_CONGESTED = new Color(220, 53, 69); // Bootstrap Red
    private final Color ROAD_CONSTRUCTION = new Color(255, 165, 0); // Orange
    private final Color NODE_COLOR = new Color(150, 150, 170);
    private final Color TEXT_CYAN = new Color(0, 229, 255);
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (cityGraph != null) {
            for (Intersection node : cityGraph.values()) {
                ...
                for (Road road : node.getConnectedRoads()) {
                    ...
                    if (road.isUnderConstruction()) {
                        g2d.setColor(ROAD_CONSTRUCTION);
                    } else if (road.getTrafficDensity() > 0.6) {
                        g2d.setColor(ROAD_CONGESTED); 
                    } else {
                        g2d.setColor(ROAD_NORMAL);
                    }
            ...
            for (Intersection node : cityGraph.values()) {
                ...
                if (node.isHorizontalGreen) {
                    g2d.setColor(new Color(0, 255, 100)); // Neon Green
                    g2d.fillRect(cx - 6, cy - 2, 2, 4); // left
                    g2d.fillRect(cx + 4, cy - 2, 2, 4); // right
                    
                    g2d.setColor(new Color(255, 50, 50)); // Neon Red
        ...
        if (activeVehicles != null) {
            for (Vehicle v : activeVehicles) {
                ...
                // Priority: Ambulance > Red (Waiting/Stuck) > Magenta (Smart Detour) > Cyan (Normal)
                if (v.isAmbulance) {
                    if ((v.ticksAlive / 10) % 2 == 0) g2d.setColor(new Color(255, 0, 0)); // Flash Red
                    else g2d.setColor(new Color(0, 0, 255)); // Flash Blue
                } else if (v.isWaitingOrSlow) {
                    g2d.setColor(new Color(255, 60, 60)); // Bright Red
                } else if (v.hasRerouted) {
                    g2d.setColor(new Color(255, 0, 255)); // Neon Magenta
                } else {
                    g2d.setColor(new Color(0, 200, 255)); // Bright Cyan
                }
